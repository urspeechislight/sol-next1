...
Python exits with code 2 and a clear message, blocking the tool call rather
than silently bypassing the guardrails (fail-closed).
...
invocation latency low and avoid coupling to `uv sync` state. Handlers have
zero third-party deps for this reason.
...
Decisions carry: `rule_id`, `handler`, `why`, `fix`, optional `doc` link.
The dispatcher emits a JSON envelope on stderr (machine-parseable) plus a
...
Handler order matters: cheap structural checks first, then content scans,
then size caps. The first deny in the chain blocks.

## Adding a handler

1. Create `.claude/lib/handlers/my_rule.py` exporting a `check(ctx) -> Decision`.
...
Every decision (block, advisory, or allow that fired some checks) is
appended as a JSON line to `.claude/audit/events.jsonl`. The log is
gitignored; ship it to your telemetry of choice.
...
CI runs this on every PR (`harness-self-test` job in `.github/workflows/ci.yml`).
...
adding the cheaper version to the harness so the agent doesn't waste
roundtrips discovering it.
